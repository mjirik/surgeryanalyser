...
python worker.py 2>&1 | \
   tee >(rotatelogs -n 3 logs/worker_$DOCKERLOGNAME_log_1.txt.bck 1M) | \
   rotatelogs -n 1 logs/worker_$DOCKERLOGNAME_log_1.txt 1M &
python worker.py 2>&1 | \
   tee >(rotatelogs -n 3 logs/worker_$DOCKERLOGNAME_log_2.txt.bck 1M) | \
   rotatelogs -n 1 logs/worker_$DOCKERLOGNAME_log_2.txt 1M &

python app.py 2>&1 | \
   tee >(rotatelogs -n 3 logs/app_$DOCKERLOGNAME_log.txt.bck 1M) | \
   rotatelogs -n 1 logs/app_$DOCKERLOGNAME_log.txt 1M &
