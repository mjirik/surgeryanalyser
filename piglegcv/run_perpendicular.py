...
def main_perpendicular(filename, outputdir):
    ...
    tested_angles1 = np.linspace(-np.pi / 10., np.pi / 10., 20, endpoint=False)
    lines1 = probabilistic_hough_line(edges, threshold=5, line_length=25,
                                 line_gap=5, theta=tested_angles1)

    tested_angles2 = np.linspace(np.pi/2.-np.pi/10. , np.pi/2.+np.pi/10., 10, endpoint=False)
    lines2 = probabilistic_hough_line(edges, threshold=20, line_length=45,
